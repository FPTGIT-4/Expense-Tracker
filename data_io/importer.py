...
class DataImporter:
    def __init__(self, user, file_obj, is_restore=False):
        ...
        self.imported_counts = {
            ...
            'transaction_history': 0, 'user_settings': 0
        }
    ...
    def _get_expected_headers(self):
        ...
        return {
            ...
            'transaction_history': ['history id', 'activity type', 'amount'],
            'user_settings': ['setting id', 'currency', 'budget threshold']
        }
    ...
    def import_data(self):
        ...
        with transaction.atomic():
            ...
            # 9. Transaction History (No-op as history is auto-generated by signals during import)
            self._import_transaction_history(wb)
    ...
    def _import_user_settings(self, wb):
        headers, rows = self._get_sheet_rows(wb, 'user_settings')
        if not headers:
            return

        currency_idx = self._find_col_idx(headers, ['currency'])
        threshold_idx = self._find_col_idx(headers, ['budget threshold', 'threshold'])
        enable_alerts_idx = self._find_col_idx(headers, ['enable budget alerts', 'enable_budget_alerts'])
        low_bal_alerts_idx = self._find_col_idx(headers, ['low balance alerts', 'low_balance_alerts'])
        show_badge_idx = self._find_col_idx(headers, ['show navbar badge', 'navbar_badge'])
        show_banner_idx = self._find_col_idx(headers, ['show dashboard banner', 'dashboard_banner'])
        show_panel_idx = self._find_col_idx(headers, ['show dashboard panel', 'dashboard_panel'])
        alert_scope_idx = self._find_col_idx(headers, ['alert scope', 'scope'])
        def_min_idx = self._find_col_idx(headers, ['default minimum', 'minimum balance'])
        dark_mode_idx = self._find_col_idx(headers, ['dark mode', 'theme'])

        for r_num, row in rows:
            try:
                settings, created = UserSettings.objects.get_or_create(user=self.user)
                
                if currency_idx != -1 and row[currency_idx] is not None:
                    settings.currency = str(row[currency_idx]).strip()
                if threshold_idx != -1 and row[threshold_idx] is not None:
                    settings.budget_threshold = int(row[threshold_idx])
                if enable_alerts_idx != -1 and row[enable_alerts_idx] is not None:
                    settings.enable_budget_alerts = str(row[enable_alerts_idx]).strip().lower() in ('true', '1', 'yes', 'on')
                if low_bal_alerts_idx != -1 and row[low_bal_alerts_idx] is not None:
                    settings.low_balance_alerts = str(row[low_bal_alerts_idx]).strip().lower() in ('true', '1', 'yes', 'on')
                if show_badge_idx != -1 and row[show_badge_idx] is not None:
                    settings.low_balance_show_navbar_badge = str(row[show_badge_idx]).strip().lower() in ('true', '1', 'yes', 'on')
                if show_banner_idx != -1 and row[show_banner_idx] is not None:
                    settings.low_balance_show_dashboard_banner = str(row[show_banner_idx]).strip().lower() in ('true', '1', 'yes', 'on')
                if show_panel_idx != -1 and row[show_panel_idx] is not None:
                    settings.low_balance_show_dashboard_panel = str(row[show_panel_idx]).strip().lower() in ('true', '1', 'yes', 'on')
                if alert_scope_idx != -1 and row[alert_scope_idx] is not None:
                    settings.low_balance_alert_scope = str(row[alert_scope_idx]).strip()
                if def_min_idx != -1 and row[def_min_idx] is not None:
                    settings.low_balance_default_minimum = parse_decimal(row[def_min_idx])
                if dark_mode_idx != -1 and row[dark_mode_idx] is not None:
                    settings.dark_mode = str(row[dark_mode_idx]).strip().lower() in ('true', '1', 'yes', 'on')
                
                settings.save()
                self.imported_counts['user_settings'] = 1
            except Exception as e:
                self.skipped_invalid += 1
                self.errors.append({"sheet": "User Settings", "row": r_num, "error": str(e), "data": str(row)})

    def _import_transaction_history(self, wb):
        # Transaction history is automatically populated by Django post_save and post_delete signals
        # on Accounts, Incomes, Expenses, and Transfers. No manual import is needed to prevent duplication.
        pass
